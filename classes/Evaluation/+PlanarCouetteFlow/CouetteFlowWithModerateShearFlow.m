classdef CouetteFlowWithModerateShearFlow < Evaluation
    methods
        function this = CouetteFlowWithModerateShearFlow()
            ...
            for measure = elements.group(@(e) e.Measurable)
                for integrator = measure.Elements.group(@(e) e.Integrator)
                    for thermostat = integrator.Elements.group(@(e) e.Thermostat)
                        ...
                        for n = thermostat.Elements.group(@(e) e.N)
        ...
        function filename = savePlot(this, plot)
            filename = sprintf("figures/PlanarCouetteFlow/UncorrelatedCouetteFlow_%s", plot.ID);
        end
    end
end